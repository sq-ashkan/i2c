// CRC-8 polynomial 0x31 (HTU31 standard)
#define CRC8_POLY 0x31

// 0x31 === 00110001
// X^7 + X^6 + X^5 + X^4 + X^3 + X^2 + X^1 + X^0
// X^5 + X^4 + X^0
// 00000000 => 0x00
// 00000001 => 0x31
// 00000011 => ((X^1 + X^0) * X^8) mod (X^5 + X^4 + X^0) = X^6 + X^4 + X^1 + X^0 = 01010011 = 0x53

// Pre-calculated CRC-8 lookup table
static const uint8_t crc8_table[256] = { // from 0x31 algo
    };

// Calculate CRC-8 for data
uint8_t crc8_calculate(const uint8_t *data, size_t length)
{
    uint8_t crc = 0x00;

    for (size_t i = 0; i < length; i++)
    {
        crc = crc8_table[crc ^ data[i]]; // XOR -> b1 .. bn
    }

    return crc;
}

// Calculate CRC-8 for serial number (24-bit)
uint8_t crc8_calculate_serial(uint32_t serial_number, uint8_t num_bytes)
{
    uint8_t data[4];
    data[0] = (serial_number >> 24) & 0xFF;
    data[1] = (serial_number >> 16) & 0xFF;
    data[2] = (serial_number >> 8) & 0xFF;
    data[3] = serial_number & 0xFF;

    return crc8_calculate(data + (4 - num_bytes), num_bytes); // data + 2 = data[2]
}